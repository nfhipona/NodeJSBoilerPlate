...
exports.fileExtension = (fromFilename) => {
    return path.extname(fromFilename);
}

exports.fileName = (name, ext) => {
    return `${name}${ext}`;
}
...
exports.config = (folder_path, filter) => {
    ...
    const diskStorage = multer.diskStorage({
        ...
        filename: function (req, file, cb) {
            const extension = exports.fileExtension(file.originalname);
            const filename = exports.fileName(`${file.fieldname}-${Date.now()}`, extension);
            cb(null, filename);
        }
    });
