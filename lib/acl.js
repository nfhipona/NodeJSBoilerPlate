...
module.exports = (database) => {
    ...
    const acl = (resource, mode, skip_check = false) => (req, res, next) => {
        ...
        function _proceed() {
            ...
            if (skip_check && (roleCode.isEqualTo('sup_admin') || 
                roleCode.isEqualTo('sys_admin'))) return next(); // skip validation

            is_maintenance(req, res, () => {
                database.connection((err, conn) => {
                    if (err) return helper.sendError(conn, res, err, c.DATABASE_CONN_ERROR);
                    _validate_permission(conn);
                });
            });
        }

        function _validate_permission(conn) {
            const fields = [
                's.id AS resource_id',
            ...
            if (roleId.length > 0) {
                where.push(`p.role_id = ${database.uuidToBIN}`);
                values.push(roleId);
            }else{
                where.push(`r.code = ?`);
                values.push(`std_user`);
            }
    ...
    const login_check = (resource, mode) => (req, res, next) => {
        ...
        function _proceed() {
            if (!database || !resource || !mode) return helper.send500(null, res, null, c.SERVER_ERROR);
            if (!roleCode || roleCode == null) return helper.send401(null, res, null, c.UNAUTHORIZED_REQUEST);
            if (roleCode.isEqualTo('sup_admin') || roleCode.isEqualTo('sys_admin')) return helper.send200(null, res, data, c.USER_SIGNIN_SUCCESS); // skip validation
    ...
    /*
     * Route maintenance check -- no need to implement on endpoints that has other acl middleware integration
    ...
    const is_maintenance = (req, res, next) => {
        ...
        function _is_server_down(conn) {
            ...
            const query = `SELECT ${fields} FROM maintenance m`;
            ...
            conn.query(query, null, (err, rows) => {
                ...
                if (is_down) {
                    const response_message = helper.errMsgData(503, c.SERVER_MAINTENANCE);
                    response_message.info = info;
                    return helper.send503(conn, res, response_message, c.SERVICE_UNAVAILABLE);
                }
