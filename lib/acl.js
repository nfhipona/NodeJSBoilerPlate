...
module.exports = (database) => {
    ...
    const acl = (resource, mode, skip_check = false) => (req, res, next) => {
        ...
        function _validate_permission(conn) {
            ...
            const where = [
                `(p.role_id = ${database.uuidToBIN} OR r.code = ?)`,
                `LOWER(s.code) = LOWER(?)`,
                `LOWER(p.mode) = LOWER(?)`
            ].join(' AND ');
            ...
            conn.query(query, [roleId, `std_user`, resource, mode], (err, rows) => {
    ...
    const login_check = (resource, mode) => (req, res, next) => {
        ...
        function _proceed() {
            if (!database || !resource || !mode) return helper.send500(null, res, err, c.SERVER_ERROR);
            if (roleCode.isEqualTo('sup_admin') || 
                roleCode.isEqualTo('sys_admin')) return helper.send200(null, res, data, c.USER_SIGNIN_SUCCESS); // skip validation
