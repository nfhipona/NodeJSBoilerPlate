...
module.exports = (database) => {
    ...
    const acl = (resource, mode, skip_check = false) => (req, res, next) => {
        ...
        function _validate(conn) {
            ...
            const where = [
                `p.role_id = ${database.uuidToBIN(roleId)}`,
                `LOWER(s.code) = LOWER(?)`,
                `LOWER(p.mode) = LOWER(?)`
            ].join(' AND ');
            ...
            conn.query(query, [resource, mode], (err, rows) => {
    ...
    const login_check = (resource, mode) => (req, res, next) => {
        ...
        function _validate(conn) {
            ...
            conn.query(query, [resource, mode], (err, rows) => {
                ...
                const record = rows[0];
                if (record.resource_disabled === 1) return helper.send503(conn, res, err, c.SERVICE_UNAVAILABLE); // check if resource is deleted - service is unavailable
                if (record.access_disabled === 1) return helper.send401(conn, res, err, c.UNAUTHORIZED_REQUEST); // check if resource access is disabled
