...
module.exports = (api, database, auth) => {
    ...
    api.post    ('/users/accounts',                                 is_maintenance,                                 auth.verifyToken(c.USER_TOKEN),                             acl('user_account', 'w'),                                   user.create_account);
    api.put     ('/users/accounts',                                 is_maintenance,                                 auth.verifyToken(c.USER_TOKEN),                             acl('user_account', 'w'),                                   user.update_account);

    /** UPLOAD */
    const upload            = uploadJS(database, auth);

    api.post    ('/uploads/avatar',                                 is_maintenance,                                 auth.verifyToken(c.USER_TOKEN),                             acl('media_resource', 'w'),                                 upload.avatar_upload);
}