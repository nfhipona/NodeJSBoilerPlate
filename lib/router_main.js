...
module.exports = (api, database, auth) => {
    ...
    api.post    ('/users/accounts',                                 is_maintenance,                                 acl('user_account', 'w'),                                   auth.verifyToken(c.USER_TOKEN),                             user.create_account);
    api.put     ('/users/accounts',                                 is_maintenance,                                 acl('user_account', 'w'),                                   auth.verifyToken(c.USER_TOKEN),                             user.update_account);

    /** UPLOAD */
    const upload            = uploadJS(database, auth);

    api.post    ('/uploads/avatar',                                 is_maintenance,                                 acl('media_resource', 'w'),                                 auth.verifyToken(c.USER_TOKEN),                             upload.avatar_upload);
}