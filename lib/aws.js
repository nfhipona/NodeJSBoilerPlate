...
function loadAWSFunctions() {
    ...
    function s3Upload(key, file_path, cb) {
        ...
        // call S3 upload file to specified bucket
     ...
     * 
     * @param {*} key file name
     * @param {*} range The request specifies the range header to retrieve a specific byte range. `bytes=0-9` -- set null if full range
     * @param {*} cb callback -> (err, data)
     */
     function s3GetObject(key, range, cb) {
        // create get object params
        const params = range ? {
            Bucket: bucketConfig.bucket,
            Key: key,
            Range: range
     ...
     * @param {*} cb callback -> (err, data)
     */
     function s3GetObjectFrom(bucket, key, range, cb) {
        // create get object params
        const params = range ? {
            Bucket: bucket,
            Key: key,
     ...
     * @param {*} range The request specifies the range header to retrieve a specific byte range. `bytes=0-9` -- set null if full range
     */
     function s3GetObjectReadStream(key, range) {
        // create get object params
        const params = range ? {
            Bucket: bucketConfig.bucket,
            Key: key,
     ...
     * @param {*} range The request specifies the range header to retrieve a specific byte range. `bytes=0-9` -- set null if full range
     */
     function s3GetObjectReadStreamFrom(bucket, key, range) {
        // create get object params
        const params = range ? {
            Bucket: bucket,
            Key: key,
